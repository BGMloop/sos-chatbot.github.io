 ...
 */
export function useNotificationVolume(): [number, (volume: number) => void] {
  ...
  const setVolume = (newVolume: number) => {
    ...
    if (typeof window !== 'undefined' && (window as any).setNotificationVolume) {
      (window as any).setNotificationVolume(newVolume);
    }
  };
