...
import { Book, Copy, Check, Smartphone, Settings2 } from "lucide-react";
export function BookFormatting({ content, title = "AI Response", author = "SOS Chatbot", onRetry }) {
    const [copied, setCopied] = useState(false);
    const [showSettings, setShowSettings] = useState(false);
    const [formatStyle, setFormatStyle] = useState('book');
    const [activeSection, setActiveSection] = useState('content');
    const [messageTheme, setMessageTheme] = useState('ios');
    ...
    const renderTitlePage = () => (<div className="min-h-[300px] flex flex-col items-center justify-center text-center p-8 border-b border-gray-200">
    ...
    const renderChapter = (chapter, index) => {
        ...
        return (<div key={index} className="mb-8 page-break-before">
        ...
        <div className="max-w-none">
          <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeRaw]} components={{
                ...
                p: ({ children }) => {
                    ...
                    if (isPoetry) {
                        return (<div className="verse-format my-4">
                    ...
                    return (<p className={`${dropCapEnabled && index === 0 ? 'first-letter-drop' : 'indent-6'} mb-4 leading-relaxed`}>
                    {children}
                  </p>);
                },
                // Handle blockquotes with special styling
                blockquote: ({ children }) => (<blockquote className="book-blockquote">
                  {children}
                </blockquote>),
                // Format poetry/verse with hanging indents
                pre: ({ children }) => (<pre className="whitespace-pre-wrap my-6 pl-8 pr-4 text-sm border-l-2 border-r-2 border-gray-200 py-4 font-serif">
                  {children}
                </pre>),
                // Style headings within chapters
                h3: ({ children }) => (<h3 className={`text-xl font-${fontFamily} font-semibold mt-8 mb-4`}>{children}</h3>),
                // Style dialogue with hanging indents
                li: ({ children }) => (<li className="pl-4 -indent-4 mb-2">{children}</li>),
                ...
                img: ({ src, alt }) => (<div className="my-6 text-center">
    ...
    const renderBookContent = () => (<div className={`bg-cream font-${fontFamily} text-gray-900 px-6 pt-4 pb-8`}>
      {activeSection === 'title' ? renderTitlePage() : (<div>
          {chapters.map((chapter, index) => renderChapter(chapter, index))}
        </div>)}
    </div>);
    const renderModernContent = () => (<div className="bg-white text-gray-900 px-6 pt-4 pb-8">
    ...
    const renderMessageBubble = () => {
        ...
        return (<div className={`max-w-md mx-auto ${messageTheme === 'ios' ? 'font-sans' : ''}`}>
    ...
    const renderSettings = () => (<div className="bg-white border-t border-gray-200 p-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        {formatStyle === 'book' && (<>
            ...
            <div className="flex items-center">
              <input type="checkbox" id="dropCap" checked={dropCapEnabled} onChange={() => setDropCapEnabled(!dropCapEnabled)} className="mr-2"/>
              <label htmlFor="dropCap" className="text-sm text-gray-700">Enable Drop Caps</label>
            </div>
          </>)}
        
        {formatStyle === 'message' && (<div>
            <label className="text-sm text-gray-700 block mb-1">Message Theme</label>
            <select value={messageTheme} onChange={(e) => setMessageTheme(e.target.value)} className="w-full p-2 border rounded">
    ...
    return (<div className="w-full max-w-4xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
      {/* Format Style Tabs */}
      <div className="flex items-center justify-between border-b border-gray-200 px-4 py-2 bg-gray-50">
        <div className="flex space-x-2">
          <Button variant={formatStyle === 'book' ? "default" : "ghost"} size="sm" onClick={() => setFormatStyle('book')} className="text-xs">
            <Book className="h-4 w-4 mr-1"/>
            Book
          </Button>
          <Button variant={formatStyle === 'modern' ? "default" : "ghost"} size="sm" onClick={() => setFormatStyle('modern')} className="text-xs">
            Modern
          </Button>
          <Button variant={formatStyle === 'message' ? "default" : "ghost"} size="sm" onClick={() => setFormatStyle('message')} className="text-xs">
        ...
        <div className="flex items-center space-x-2">
          {formatStyle === 'book' && (<>
              <Button variant={activeSection === 'title' ? "default" : "ghost"} size="sm" onClick={() => setActiveSection('title')} className="text-xs">
                Title Page
              </Button>
              <Button variant={activeSection === 'content' ? "default" : "ghost"} size="sm" onClick={() => setActiveSection('content')} className="text-xs">
                Content
              </Button>
            </>)}
          
          <Button onClick={() => setShowSettings(!showSettings)} variant="ghost" size="sm" className="text-xs">
            <Settings2 className="h-4 w-4 mr-1"/>
            Settings
          </Button>
          
          <Button onClick={copyToClipboard} variant="ghost" size="sm" className="text-xs">
            {copied ? (<><Check className="h-3 w-3 mr-1"/> Copied</>) : (<><Copy className="h-3 w-3 mr-1"/> Copy</>)}
          </Button>
          
          {onRetry && (<Button onClick={onRetry} variant="ghost" size="sm" className="text-xs">
              Retry
            </Button>)}
        </div>
      </div>
