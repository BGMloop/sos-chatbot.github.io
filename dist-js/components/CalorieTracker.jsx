...
export default function CalorieTracker() {
    ...
    const NutrientCircle = ({ value, label, color, unit = "g" }) => {
        ...
        if (label === "Carb")
            percentage = Math.min(100, value * 1.5);
        if (label === "Protein")
            percentage = Math.min(100, value * 2);
        if (label === "Fat")
            percentage = Math.min(100, value * 3);
        ...
        <div className="relative w-28 h-28">
          ...
          <svg className="w-full h-full" viewBox="0 0 100 100">
            ...
            {/* Foreground circle */}
            <circle cx="50" cy="50" r={radius} fill="transparent" stroke={color} strokeWidth="8" strokeDasharray={circumference} strokeDashoffset={strokeDashoffset} strokeLinecap="round" transform="rotate(-90 50 50)"/>
          </svg>
          {/* Center text */}
      ...
      <Card className="shadow-lg">
        ...
        <CardContent className="pt-6">
          <div className="space-y-4">
            {/* Image Upload Area */}
            <div ref={dropZoneRef} className={`border-2 ${isDragging ? 'border-blue-500 bg-blue-50' : 'border-dashed border-gray-300'} rounded-lg p-4 text-center hover:bg-gray-50 transition-colors`} onDragEnter={handleDragEnter} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
              {selectedImage ? (<div className="relative">
                  <img src={selectedImage} alt="Food" className="max-h-[300px] mx-auto rounded-md"/>
                  <button className="absolute top-2 right-2 bg-red-500 text-white p-1 rounded-full hover:bg-red-600" onClick={clearImage}>
                    <X size={16}/>
                  </button>
                  ...
                  <div className="flex justify-center gap-4 mt-4">
                    ...
                    <Button variant="outline" onClick={handleCapture} className="flex items-center gap-2">
                      <Camera size={18}/>
                      Take Photo
                    </Button>
                  </div>
            ...
            {/* Notes Field */}
            <div>
              <Textarea placeholder="Add any notes about the food (optional)..." value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full h-24 resize-none"/>
            </div>
            
            {/* Analyze Button */}
            <Button onClick={analyzeImage} disabled={!selectedImage || isAnalyzing} className="w-full bg-blue-600 hover:bg-blue-700">
              {isAnalyzing ? (<>
        ...
        {/* Results Section */}
        {result && (<CardContent className="border-t border-gray-200 pt-6 mt-4">
            <div className="space-y-6">
              ...
              {selectedImage && (<div className="relative max-w-md mx-auto mb-8">
                  <img src={selectedImage} alt="Analyzed Food" className="w-full rounded-xl shadow-md"/>
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 shadow-sm">
                ...
                <div className="flex justify-around items-center gap-4 px-4">
                  ...
                  <NutrientCircle value={((_a = result.nutritionFacts) === null || _a === void 0 ? void 0 : _a.carbs) || 0} label="Carb" color="#4ade80" // Green
        />
                  
                  <NutrientCircle value={((_b = result.nutritionFacts) === null || _b === void 0 ? void 0 : _b.protein) || 0} label="Protein" color="#60a5fa" // Blue
        />
                  
                  <NutrientCircle value={((_c = result.nutritionFacts) === null || _c === void 0 ? void 0 : _c.fat) || 0} label="Fat" color="#f97316" // Orange 
        />
                </div>
              </div>
                ...
                <TabsContent value="summary" className="p-4 border rounded-lg mt-2">
                  <div className="space-y-2">
                    ...
                    <p className="text-gray-700">
                      ...
                      {(_d = result.nutritionFacts) === null || _d === void 0 ? void 0 : _d.protein}g protein, {(_e = result.nutritionFacts) === null || _e === void 0 ? void 0 : _e.carbs}g carbs, 
                      and {(_f = result.nutritionFacts) === null || _f === void 0 ? void 0 : _f.fat}g fat.
                    </p>
                  </div>
                </TabsContent>
        ...
        <CardFooter className="bg-gray-50 px-6 py-4 flex justify-between">
          ...
          {selectedImage && !result && !isAnalyzing && (<Button onClick={analyzeImage} variant="outline" size="sm">
              Analyze Now
            </Button>)}
        </CardFooter>
      </Card>
      ...
      <Card className="mt-6 shadow-md">
        ...
        <CardContent>
          <div className="flex items-center gap-2 border rounded-lg p-2">
            <Input placeholder="Describe your meal (optional)..." className="border-0 focus-visible:ring-0"/>
            
            {/* File Upload Button with Paperclip Icon */}
            <Button variant="ghost" size="icon" className="flex items-center justify-center text-center font-medium cursor-pointer outline-hidden focus-visible:ring-3 relative whitespace-nowrap transition-colors focus-visible:ring-default focus-visible:ring-offset-1 aria-disabled:text-hint aria-disabled:bg-state-disabled aria-disabled:cursor-not-allowed aria-busy:cursor-wait aria-busy:text-transparent bg-state-soft hover:bg-state-soft-hover active:bg-state-soft-press gap-1 text-sm p-0 rounded-md size-8 text-icon-default-subtle" aria-label="Add files" onClick={handleUpload}>
            ...
            {/* Camera Button */}
            <Button variant="ghost" size="icon" className="flex items-center justify-center text-center font-medium cursor-pointer outline-hidden focus-visible:ring-3 relative whitespace-nowrap transition-colors focus-visible:ring-default focus-visible:ring-offset-1 aria-disabled:text-hint aria-disabled:bg-state-disabled aria-disabled:cursor-not-allowed aria-busy:cursor-wait aria-busy:text-transparent bg-state-soft hover:bg-state-soft-hover active:bg-state-soft-press gap-1 text-sm p-0 rounded-md size-8 text-icon-default-subtle" aria-label="Take photo" onClick={handleCapture}>
              <Camera size={18}/>
            </Button>
            
            <Button variant="default" size="sm" className="bg-blue-600 hover:bg-blue-700" disabled={!selectedImage || isAnalyzing} onClick={analyzeImage}>
              Analyze
            </Button>
          </div>
        </CardContent>
