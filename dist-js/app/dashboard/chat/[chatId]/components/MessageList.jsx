    ...
    if (content.includes("\n1.") || content.match(/\n\d+\./)) {
        ...
        {content.split('\n').map((line, index) => {
                ...
                if (listMatch) {
                    ...
                    return (<div key={index} className="flex items-start py-1 -mx-1">
...
const MessageItem = memo(({ message }) => {
    ...
    return (<div className={cn("flex items-start gap-3 p-3 rounded-lg", isUser ? "bg-blue-50 dark:bg-blue-900/20" : "bg-gray-50 dark:bg-gray-800/50")}>
      {isUser ? (<div className={cn("h-8 w-8 rounded-full flex items-center justify-center text-white", "bg-blue-500")}>
          U
        </div>) : (<div className="h-10 w-10 rounded-full overflow-hidden flex-shrink-0">
          <Image src="/logo/logo.png" alt="Assistant" width={40} height={40} className="object-cover"/>
        </div>)}
      <div className="flex-1 min-w-0">
        <div className="font-medium text-sm mb-1 flex items-center">
          <span>{isUser ? "You" : "Assistant"}</span>
          {message.timestamp && (<span className="text-xs text-gray-400 ml-2">
