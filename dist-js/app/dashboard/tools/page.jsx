...
export default function ToolsTestPage() {
    ...
    const [selectedTool, setSelectedTool] = useState("math");
        ...
        async function loadToolStatus() {
            try {
                ...
                if (response.ok) {
                    ...
                    Object.entries(data.results).forEach(([id, info]) => {
    ...
    const renderToolForm = () => {
        switch (selectedTool) {
            ...
            <Textarea id="math-input" value={mathInput} onChange={(e) => setMathInput(e.target.value)} placeholder="Enter a math expression or question (e.g., 2+2 or solve x^2 + 2x + 1 = 0)" className="min-h-[100px]"/>
          </div>);
            case 'exchange':
                return (<div className="grid gap-4">
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <Input id="currency-from" value={currencyFrom} onChange={(e) => setCurrencyFrom(e.target.value)} placeholder="USD"/>
              </div>
              <div>
                <Label htmlFor="currency-to">To Currency</Label>
                <Input id="currency-to" value={currencyTo} onChange={(e) => setCurrencyTo(e.target.value)} placeholder="EUR"/>
              </div>
              <div>
                <Label htmlFor="currency-amount">Amount (Optional)</Label>
                <Input id="currency-amount" value={currencyAmount} onChange={(e) => setCurrencyAmount(e.target.value)} placeholder="100" type="number"/>
              </div>
            </div>
          </div>);
            case 'open_meteo_weather':
            ...
            <Input id="weather-location" value={weatherLocation} onChange={(e) => setWeatherLocation(e.target.value)} placeholder="Enter a city or location (e.g., New York, London, Tokyo)"/>
          </div>);
            case 'news':
                return (<div className="grid gap-4">
            <Label htmlFor="news-topic">Topic</Label>
            <Input id="news-topic" value={newsTopic} onChange={(e) => setNewsTopic(e.target.value)} placeholder="Enter a news topic (e.g., technology, climate change, sports)"/>
          </div>);
            case 'news_headlines':
                return (<div className="grid gap-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input id="news-country" value={newsCountry} onChange={(e) => setNewsCountry(e.target.value)} placeholder="us"/>
    ...
    const getStatusBadge = (toolId) => {
        var _a;
        const status = ((_a = toolStatus[toolId]) === null || _a === void 0 ? void 0 : _a.status) || 'unknown';
        if (status === 'available') {
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid gap-6">
            ...
            {error && (<div className="p-4 rounded-md bg-red-50 text-red-800 dark:bg-red-900/20 dark:text-red-400">
