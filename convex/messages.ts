...
// Add type declaration inline to avoid needing @types/node-fetch
// @ts-ignore - ignore missing types for node-fetch
import fetch from "node-fetch";

const SHOW_COMMENTS = true;

// Tool schemas for WXflows integration
const TOOL_SCHEMAS: { [key: string]: string } = {
...
async function executeWxTool(toolName: string, params: any): Promise<string> {
  try {
    ...
    if (toolName === "open_meteo_weather") {
      ...
      const weatherData: {[key: string]: {temp: string, feels: string, condition: string, humidity: string, wind: string}} = {
        "new york": {
          temp: "44°F", 
          feels: "41°F", 
          condition: "Clear", 
    ...
    if (toolName === "math") {
      ...
      const locationKey = Object.keys(locations).find(loc => 
        params.location.toLowerCase().includes(loc)
      ) || "dallas";
    ...
    try {
      ...
      // Special case for AI news
        ...
        for (const msg of messages) {
          if (msg.role === "user" && (
              msg.content.toLowerCase().includes("derivative") || 
